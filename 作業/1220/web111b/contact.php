  ...
  <table align="center">
    <tr>
      <td align="left">
        <fieldset>
          <legend>
            <h3 align="center">聯絡表單</h3>
          </legend>
          <form name="form" action="contact.php" method="post">
            <label>
              姓名：<input type="text" name="name" required="required" pattern="[^%!&+'?<>/\\|^0-9]"><br>
              <br>
            </label>
            <label>
              ...
              <!-- 因為世界各地電話號碼格式不同, 使用tel並不會特別驗證資料格式, 若要規定格式還是要用正規表達式限制 -->
              <br>
            </label>
            <label>
...
if (isset($_POST['name']) && isset($_POST['tel']) && isset($_POST['request'])) {
    ...
    // } else {
    echo "姓名：" . $name . "<br>" . "電話：" . $tel . "<br>" . "意見：" . $request . "<br>";
    // }
}
?>
      </td>
